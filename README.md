...
- 按住`x2`为循环点击`F`键，可用于自动交互和对话，松开停止。
- 点击`x1`为自动登龙，目前仅支持作者测试的`单手剑` `双手剑`和`长柄武器`角色和`鹿野院平藏`
...
- 自动登龙延时可能需要根据情况微调，延时为0代表不登龙：
    1. 参照其他直接修改`character.json`，单位为毫秒。
    2. 也可修改`GenshinMacro.py`中的`debug = True`和`port`
       ...
       ，消息接收后自动保存。
- 该脚本需要有鼠标侧键，如果没有鼠标侧键请将`Callback.py`中的`x1`、`x2`
  修改为想要设定的键位，若要设为键盘键位的话还需要将修改处附近的`mouse`改为`keyboard`。
